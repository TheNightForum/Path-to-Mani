...
public class PartMan {
    ...
    public void shieldSpark(PtmGame game, Vector2 collPos, Hull hull, TextureAtlas.AtlasRegion shieldTex, float perc) {
        if (perc <= 0) {
            return;
        }
    ...
    public RectSprite blip(PtmGame game, Vector2 pos, float angle, float sz, float fadeTime, Vector2 spd,
        ...
        RectSprite s = new RectSprite(tex, sz, 0, 0, new Vector2(), DraLevel.PART_FG_0, angle, 0, PtmColor.WHITE, true);
        ArrayList<Dra> dras = new ArrayList<Dra>();
        dras.add(s);
