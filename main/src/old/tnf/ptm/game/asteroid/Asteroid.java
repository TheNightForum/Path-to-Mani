...
public class Asteroid implements PtmObject {
    ...
    public Asteroid(PtmGame game, TextureAtlas.AtlasRegion tex, Body body, float size, RemoveController removeController, ArrayList<Dra> dras) {
        myTex = tex;
    ...
    private boolean updateInAtm(PtmGame game) {
    ...
    public void onRemove(PtmGame game) {
        ...
        if (myLife <= 0) {
            ...
            float vol = PtmMath.clamp(mySize / .5f);
    ...
    private void throwLoot(PtmGame game, PtmItem item) {
        ...
        pos.add(myPos);
        Loot l = game.getLootBuilder().build(game, pos, item, lootSpd, Loot.MAX_LIFE, PtmMath.rnd(Loot.MAX_ROT_SPD), null);
